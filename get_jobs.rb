...
class CraigslistJobsScraper
  ...
  def gather_info
    puts "---------\n Automated SAN FRANCISCO Job Finder/Scraper/Extrodinaire \n---------"
    puts ""
    ...
    url = "http://sfbay.craigslist.org/search/jjj?query=#{search_terms}&format=rss"
    return url
  end
  ...
  def email_posters(array_of_email_addresses)
    if array_of_email_addresses
      array_of_email_addresses.each do |email|
        puts "..."
        puts email + "\n..."
        send_email(email, "Regarding your listing on Craigslist ... ") if email
      end
    end  
  end
